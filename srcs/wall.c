...
/*   Updated: 2021/03/24 16:37:41 by kyuki            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_set_wall_info(int x, t_wall_tool *tool, t_sys *s)
{
	...
	tool->texture_w = s->tex.width;
	tool->texture_h = s->tex.height;
}

static void		ft_set_tex_to_wall(int x, int y, t_wall_tool *tool, t_sys *s)
{
	tool->fr_top = y + (tool->wall_h / 2) - (s->win.h / 2);
	tool->tex_ofs_y = tool->fr_top * ((float)tool->texture_h / tool->wall_h);
	tool->texel_color = 0;
	if (s->rays[x].was_hit_vertical)
	{
		if (s->rays[x].was_hit_east == TRUE)
			tool->texel_color =
				s->tex.e[(tool->texture_w * tool->tex_ofs_y) + tool->tex_ofs_x];
		else
			tool->texel_color =
				s->tex.w[(tool->texture_w * tool->tex_ofs_y) + tool->tex_ofs_x];
	}
	else
	{
		if (s->rays[x].was_hit_north == TRUE)
			tool->texel_color =
				s->tex.n[(tool->texture_w * tool->tex_ofs_y) + tool->tex_ofs_x];
		else
			tool->texel_color =
				s->tex.s[(tool->texture_w * tool->tex_ofs_y) + tool->tex_ofs_x];
	}
	if (s->rays[x].was_hit_vertical)
